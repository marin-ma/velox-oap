...
void streamingDecompress(
  ...
  auto remaining = compressed.size();
  ...
  while (!decompressor->isFinished()) {
    // Feed a small amount each time.
    auto input_len = std::min(remaining, ramdomInputSize());
    auto output_len = decompressed.size() - decompressedSize;
    uint8_t* output = decompressed.data() + decompressedSize;
    // Decompress once.
    auto result =
        decompressor->decompress(input_len, input, output_len, output);
    ASSERT_LE(result.bytesRead, input_len);
    ASSERT_LE(result.bytesWritten, output_len);
    ASSERT_TRUE(
