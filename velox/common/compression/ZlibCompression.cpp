...
class GzipCompressor : public StreamingCompressor {
 ...
 private:
  Status init(GzipFormat format, int32_t windowBits);

  z_stream stream_{};
  int32_t compressionLevel_;
  bool initialized_{false};

  friend class GzipCodec;
};
...
Status GzipCompressor::init(GzipFormat format, int32_t windowBits) {
...
Expected<StreamingCompressor::EndResult> GzipCompressor::end(
    uint8_t* output,
    uint64_t outputLength) {
  VELOX_RETURN_UNEXPECTED_IF(
      !initialized_, Status::UserError("Called on non-initialized stream."));
...
GzipCodec::makeStreamingCompressor() {
...
Status GzipCodec::initDecompressor() {
  endCompressor();
  memset(&stream_, 0, sizeof(stream_));
...
Expected<uint64_t> GzipCodec::compress(
    const uint8_t* input,
    uint64_t inputLength,
    uint8_t* output,
    uint64_t outputLength) {
...
Expected<uint64_t> GzipCodec::decompress(
    const uint8_t* input,
    uint64_t inputLength,
    uint8_t* output,
    uint64_t outputLength) {
...
Expected<uint64_t> GzipCodec::compressFixedLength(
    const uint8_t* input,
    uint64_t inputLength,
    uint8_t* output,
    uint64_t outputLength) {
